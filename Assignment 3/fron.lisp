...
(export '(def-class do-def-class def-object do-def-object world-model-facts
           ...
           *world-model* *frontology* make-item *item-counter* *world-models*
           html-ontology do-html-ontology html-world-model))
...
(defun fact-number (fact)
  (slot-value fact 'number))

(defun fact-variables (fact)
  (remove-if-not (lambda (x) (starts-with-p (write-to-string x) "#:")) (cdr (slot-value fact 'content))))
  ...
  (let
   ((world-model (if (null label) *world-model* ; default
    ...
    (if world-model
      (progn 
        (add-element '((br)))
        (add-element `((h3) ,(format nil "Facts in ~a:" (world-model-label world-model))))
        ;(add-element (do-html-world-model-facts world-model))
        ; (print (link-facts (world-model-facts world-model)))
        (add-svg (s-dot->svg (dot-facts (world-model-facts world-model) (link-facts (world-model-facts world-model))))))
      (add-element `((h3) ,(format nil "~%Unknown world model: ~a" label))))))

(defun do-html-world-model-facts (world-model)
  (append '((table)
            ((tr)
                ((th) "number")
                ((th) "attribute")
                ((th) "value")
                ((th) "object")))
          (loop for fact in (world-model-facts world-model)
              collect (let* ((number (slot-value fact 'number))
                             (content (slot-value fact 'content))
...
(defun link-facts (facts)
  (let ((facts-done nil)
        (links nil))
    (loop for fact in facts
          when (not (find (fact-number fact) facts-done))
          do (multiple-value-bind 
               (new-links new-facts-done)
               (do-link-facts (fact-number fact) facts (adjoin (fact-number fact) facts-done))
               (setf facts-done (union new-facts-done facts-done))
               (setf links (union new-links links))))
    links))

(defun do-link-facts (my-number facts facts-done)
  (let* ((next nil)
         (links nil)
         (current-fact (find my-number facts :key #'fact-number))
         (filtered (remove-if-not (lambda (x) (not (find (fact-number x) facts-done))) facts)))
    (loop for var in (fact-variables current-fact)
          do (let* ((found (find-if (lambda (x) (find var (fact-variables x))) filtered)))
               (when found 
                 (setf next (adjoin (fact-number found) next))
                 (setf links (adjoin (list my-number (fact-number found)) links)))))
    (loop for n in next
          do (multiple-value-bind 
               (new-links new-facts-done)
               (do-link-facts n facts (adjoin n facts-done))
               (setf facts-done (union new-facts-done facts-done))
               (setf links (union new-links links))))
    (values links facts-done)))

(defun add-svg (svg)
  (add-element `((div) ,svg)))

(defun dot-facts (facts links)
  (append '(s-dot::graph ())
          (mapcar #'dot-fact facts)
          (mapcar #'dot-fact-link links)))

(defun dot-fact (fact)
  (let* ((number (slot-value fact 'number))
         (content (slot-value fact 'content))
         (attribute (first content))
         (value (second content))
         (object (third content)))
    `(s-dot::node ((s-dot::id ,(write-to-string number)) 
                   (s-dot::label ,(write-to-string content)) 
                   (s-dot::fontsize "10")))))

(defun dot-fact-link (link)
  (let ((fact1 (first link))
        (fact2 (second link)))
  `(s-dot::edge ((s-dot::from ,(write-to-string fact1)) 
                 (s-dot::to ,(write-to-string fact2))
                 (s-dot::dir "none")))))
...
(defun add-fact (content world-model &optional (time nil))
  (cond 
    ((null content) 
      'nil-content)
    ((fact-present content (world-model-facts  world-model))
...
;           (setf *predicates*
;                 (cons 
;                  (make-instance 'predicate :id label :predicate label)
;                  *predicates*)
       ;   (remainder (transform-definition ',(cdr body))) ; is for handling syntactic shorthands
...
;           (format t "~%=++ ~a" element)
       ...
       (car expression))))
    (cons arguments
          (union new-concepts (cdr expression)))))
...
(defmacro instantiate-event (&body args)
  `(do-instantiate-event ',(car args) ',(cdr args) *world-model*))
  ...
  (let
    ...
    (dolist (state states) ; no info on time yet 
      (multiple-value-bind (not-used added)
          (handle-evoked nil (second state) nil state-bindings world-model)
        (declare (ignore not-used))
        (setf state-bindings (cons (cons (car state) added) state-bindings))
        (setf added-facts (union added added-facts))))
    (dolist (action-spec actions)
      ...
      (let ((actions (cdr (assoc :action (cdr action-spec)))))
        ...
        (dolist (action actions)
              (multiple-value-bind (not-used added)
                  (handle-evoked new-action action nil state-bindings world-model time-stamp)
                (declare (ignore not-used))
                (setf added-facts (union added added-facts)))))
  ...
  (let*
      ((arg (car arg-spec))
       ...
       (item (if filled-arg ; there is already a binding 
               (if (or variable (not (every #'null (mapcar #'get-frame constraints))))
                   (cdr (assoc (second filled-arg) (world-model-objects world-model)))
                   (second filled-arg))
               (make-item arg 'individual world-model))) ; else create new one
       (variable (if variable variable item))
       (objects (world-model-objects World-model))
       (added-facts nil))
;    (format t "~%Filled: ~a ~a" ARG ITEM)
;    (if (null filled-arg) (add-object item)) ; new object is anonymous
    (setf added-facts
          (list (add-fact ; now add fact 
                 (list arg item self) world-model time)))
    (if variable
    ...
    (mapcar #'(lambda (constraint) ; add constraints about this item
                (generate-facts constraint variable (list item) bindings world-model))
            constraints)
...
(defun handle-evoked (self evoke-spec args bindings world-model &optional time)
  ...
  (let*
      ((type (second evoke-spec))
       ...
       (my-self (if variable bound-variable self))
       (new-args nil)
       (added-facts nil))
...
;    (format t "~%Handle bindings in evoked for ~a" (cddr evoke-spec))
    (dolist (arg (cddr evoke-spec))
      (if (varp arg) ; ignore is the spec of the variable
        t
        (cond
         ((not (get-frame (first arg))) ; if arg type is not a frame => arg is a direct value
          (setf new-args
                 (cons (if (varp (second arg)) `(,(first arg) ,(get-value (second arg) bindings)) arg) new-args)))
...
(defun generate-facts (concept-label self args bindings world-model &optional time)
  ...
  (let* ; first get the definition 
      ((object-def ; are we dealing with an object? 
       ...
       (objects (world-model-objects world-model))
       (individuals (world-model-individuals world-model))
       (new-fact-content nil)
;       (existing-fact (fact-present new-fact-content  (world-model-facts  world-model)))
       (added-facts nil)
       )
    (cond 
      (object-def ; we are creating a new object so that is now stored
        (add-object concept-label self world-model))
      (event-def 
        nil)
      (t
        (setf new-fact-content (cond 
                                 ((rassoc self objects) (list concept-label (first (rassoc self objects)) self))
                                 ((find self individuals) (list concept-label self self))
                                 (t nil)))))
    (setf objects (world-model-objects world-model))
    (cond
      ...
      ; sweep through all arguments to create new ones if necessary
        (dolist (arg-spec (frame-arguments frame))
          (multiple-value-bind (new-template new-bindings)
            ...
            (setf template (union new-template template))
